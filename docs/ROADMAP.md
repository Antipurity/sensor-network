    ...
    - ⋯ Reasonable defaults, decided by the user and not the handler, in separate `import`ed files, or maybe their own NPM modules (though they *are* small):
        ...
        - ⋯ `.Sensor`:
            ...
            - ✓ Read from file.
                ...
                - ⋯ Research continuous model distillation (basically [Iterated Amplification](https://www.lesswrong.com/posts/vhfATmAoJcN8RqGg6/a-guide-to-iterated-amplification-and-debate)), because restarting sucks: [shallow-ing](https://arxiv.org/abs/2106.03186) to achieve infinite depth: making models not just out of dense layers `f(f(x))` but something like `g(x) = f(x, g(x, non_trainable=true))`, where `g(x)` predicts the result (so it always tries to approximate "but what if we had 1 more layer"). `g` can be smaller than `f` and thus also distilled into a smaller net, which is distilled too, until there's only 1 layer for the whole network, or maybe ½ or less layers via matrix factorization: `x@A → x@a@b`; [this](https://2021.ecmlpkdd.org/wp-content/uploads/2021/07/sub_676.pdf) also does not let teachers diverge too far from students, though without `+` and on weights rather than on outputs. …And maybe self-distill layer width to be "infinite", maybe via just `cumsum(output)` for simplicity (can it even be called self-distillation at this point, or even ensembling?), vaguely similar to [this](https://openreview.net/forum?id=B1l6qiR5F7). (…Or just do [BYOL](https://arxiv.org/abs/2006.07733) with different online/target histories, possibly randomly dropping observations, or dropping them based on their hashes: `x@W < 0`.)
                - ⋯ An option for the per-cell sampler `(i,len)→i`, `(i,len) → (i+1)%len` by default.
                    - ⋯ Most importantly, should have a method for voting on per-cell importance, and a sampler that takes those into account after the first pass.
                    - ⋯ Research [compressing updates](http://web.cs.ucla.edu/~weiwang/paper/ICDM05_1.pdf): in a batch, measure gradient correlations (forward or backward grad? one-step, truncated, or infinite? or even [just activations](https://arxiv.org/abs/2106.08453)? or changes in internal state?), and lower the importance of most-correlated cells.
...
- ✓ Maybe, for simplicity, Python, since most ML stuff happens in Python? `send(name, data, reward=0, on_feedback=None)`, `handle(prev_feedback)→*next_data`, using only NumPy. (No need for built-in data collection nor transformation.) (The UX turned out extremely nicely. Absorbing datasets would be trivial.)
    ...
    - ⋯ Simple [linear-time infinite-window attention](https://arxiv.org/abs/2102.11174), [where the past maximizes the current layer's reward predictions (first fill actions/feedback then predict+maximize future rewards)](https://arxiv.org/abs/1509.02971).
        - ⋯ Prioritize pretraining (plain RL is too high-variance and sample-inefficient, so do it on compressed representations), such as with [BYOL](https://arxiv.org/abs/2006.07733) or [SwaV](https://paperswithcode.com/method/swav). For data 'augmentation', could mask out random or most-future-loss parts of the input, such as via prioritizing most-diverse cells in the file-reading sensor.
            - ⋯ Maximize diversity *of internal state only* (again, action-based exploration is too high-variance): [BYOL](https://arxiv.org/abs/2006.07733) extracts variances, and we can make the past maximize those (in addition to minimizing, for learning). Iffy. Research such things.
        - ⋯ Continuous distillation `g(x) = f(x, g(x, non_trainable=True))`, for lack of a reason of its non-equivalence to actual model distillation. (At inference time, could use only `g` for speed. The need to deploy is the main reason.)
    - ⋯ Numbers to optimize: a small-ish [dataset](https://pypi.org/project/datasets/), measuring reconstruction loss (possibly learned separately from representation) as a proxy for compression.
    - ⋯ A [Perceiver IO](https://arxiv.org/abs/2107.14795) model for future prediction and first-cell-number maximization.
        - ⋯ To take advantage of exponential improvement of learning, ML has to be a journey, not a series of separate steps like it is today. Sensor network being able to represent all data in one format is a prerequisite. So research lifelong learning. (Learning is fundamentally updating and so is destructive, so maybe only a slow-down of decay is possible; slowly-changing sparse update masks might be sufficient.)
        - ❌ Prediction is technically a number to optimize, so, make it the reward. (Unless all humanity instantly starts entrusting all tasks to a sensor network, we would likely have too little & too contradictory data for reward-learning to succeed.)
