    ...
    - ⋯ Reasonable defaults, decided by the user and not the handler, in separate `import`ed files, or maybe their own NPM modules (though they *are* small):
        ...
        - ⋯ `.Handler`:
            ...
            - TODO: Text (and is text really any good if each word doesn't have infinite depth, and links are everywhere):
                ...
                - All UIs for interacting with non-trivial things get more complex over time. Eventually this leads to information overload and overlap, and settings UI has to be developed, but it has an indirect effect, and is really a separate thing that's tied to language understanding and could be not implemented well. With a sensor network, very direct. ...But doesn't it have to be implemented too, anyway... So doesn't it offer no significant benefit over visual UIs, apart from being non-visual?
                - Try model-based meta-learning: `x2 = f(x1, w)`, `dx1 = df(dx2)`, but on backprop, `df` should be a neural network, and we double-backprop to make post-update weights `w - dw` (no learning rate) be more fit. To reduce memory requirements at the cost of 2× the computations, make both `f` and `df` discard and rematerialize their gradient graphs from `x1` and `dx2`. (Not sure how much of a benefit an extra layer of meta-learning really is, on top of RNNs and Transformers, though. And, try on what? Would a regular interaction dataset allow meta-learning to have any benefits?) (…Actually, Transformers really should allow meta-learning, so this is really really extra…)
