...
def act_dist(action, src, dst): # TODO: Add `noise` at the end, `None` and auto-generated by default.
...
def replay(reached_vs_timeout):
    ...
    for _ in range(replays_per_unroll): # Look, concatenation and variable-management are hard.
        ...
        # TODO: …Okay, how do we add randomness to `dist` without making the loss un-diversify it?
        #   Would "add the noise to ReplaySample, and here, make `abg` use that exact noise" be enough?
        #     …Actually, I think it may just be: not only would self-imitation not touch other-noise min actions (and neither would DDPG), but also, global-min dist would not care about noise due to its (noise-less targets) loss; so global-min-dist actions would be diverse but only be at min-dist.
