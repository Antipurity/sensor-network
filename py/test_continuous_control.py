...
#   - TODO: Implement & test 2-part loss for shortest-path-learning: `next(ev=ev(prev_action, prev_state, goal), goal=state) = action` (ground in what we actually achieve) and `next(ev=ev(prev_action, prev_state, goal), goal) = ev_delayed(action, state, goal)` (propagate how-to-achieve to the past) (`ev` is what-to-achieve-right-now, `action`-sized). Though, how to see whether we've actually achieved anything is unclear…
#   - Visualization:
